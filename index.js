...
async function getData() {
  ...
  let number = 1;
  for (const magazine of magazines.cards) {
    const card = document.createElement("div");
    card.className = "card";
    const cardTitle = document.createElement("div");
    cardTitle.className = "card-title";
    cardTitle.innerHTML = magazine.title;
    const cardMoreDetails = document.createElement("div");
    cardMoreDetails.className = "card-details";
    cardMoreDetails.innerHTML =
      '<div class="sub">$ ' +
      magazine.sub +
      " / YEAR</div><p>" +
      magazine.text +
      "</p>";
    cardMoreDetails.prepend(cardTitle);
